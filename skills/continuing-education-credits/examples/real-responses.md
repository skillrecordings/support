# Real Responses (verbatim)

## cnv_1jcyxv1h

**Question**

```text
Hi

I am in conversation with the IAAP about using these courses towards my
continuing education credits (to maintain my CPACC certification). The problem
is, they really need to know how long the training takes so they can assign the
right number of credits.

Can you help please? For example, let me/them know how long each workshop is?
```

**Answer**

```text
Hey Grace,


Sorry, I was splitting the total average. Tracking your own times Is probably best right now. 



Also, Marcy Is working with the certification team at IAAP so we can give correct and accurate credits per workshop. 


Let me know If you need anything else.


Best,
```

## cnv_1jcepd2d

**Question**

```text
Hello!

I work on the certification team at IAAP, reviewing education credit requests.  I got this question today:  can you tell me how many CAEC credits this course is worth please? It has taken me 20 hours to complete but is self paced. It is unclear how many I could submit a claim for.

We typically go off the # of hours documented on the website or on a certificate of completion.  I don't see anything like that on your website.  Do you have a suggestion on what we should award for your 6 workshops?

  1.
Foundations of Accessibility
  2.
Design Thinking and People Skills for Accessibility
  3.
Manual Accessibility Testing
  4.
Semantic markup with HTML and ARIA
  5.
Coding Accessible Interactions and Mechanics
  6.
Automated Accessibility Testing

Some examples from other providers:

  *
We award 15 credits (total of 45) for the DHS trusted tester
  *
Deque has the # of credits earned on each of their course completion certificates.

Any thoughts you have would be greatly appreciated.


United in Accessibility


Ellen Strom

Certification Program Assistant, Global Initiative for Inclusive ICTs (G3ict)

International Association of Accessibility Professionals (IAAP), a Division of G3ict

LinkedIn<https://www.linkedin.com/company/international-association-of-accessibility-professionals/posts/?feedView=all> | G3ict<http://www.g3ict.org/> | IAAP<http://www.accessibilityassociation.org/> | M-Enabling<https://m-enabling.com/>

United States |Eastern Time Zone

[cid:e6098980-a2b1-4ff5-96fd-84eafb533aea]
 Register now<https://www.m-enabling.com/event/a75cd7b0-6c94-4ad6-9f5c-05110c32b04d/registration>
```

**Answer**

```text
— Haze ----------- Forwarded message ----------- From: [EMAIL] Date: December 17, 2025 at 5:43 AM PST Subject: Marcy Sutton Testing Accessibility Course To: [EMAIL] Hello! I work on the certification team at IAAP, reviewing education credit requests.&nbsp; I got this question today:&nbsp; can you tell me how many CAEC credits this course is worth please? It has taken me 20 hours to complete but is self paced. It is unclear how many I could submit a claim for. We typically go off the # of hours documented on the website or on a certificate of completion.&nbsp; I don't see anything like that on your website.&nbsp; Do you have a suggestion on what we should award for your 6 workshops? Foundations of Accessibility Design Thinking and People Skills for Accessibility Manual Accessibility Testing Semantic markup with HTML and ARIA Coding Accessible Interactions and Mechanics Automated Accessibility Testing Some examples from other providers: We award 15 credits (total of 45) for the DHS trusted tester Deque has the # of credits earned on each of their course completion certificates. Any thoughts you have would be greatly appreciated. U nited in Accessibility&nbsp;&nbsp; Ellen Strom Certification Program Assistant, Global Initiative for Inclusive ICTs (G3ict)&nbsp; International Association of Accessibility Professionals (IAAP), a Division of G3ict&nbsp;&nbsp; LinkedIn &nbsp;| G3ict &nbsp;| IAAP &nbsp;| M-Enabling &nbsp; United States |Eastern Time Zone &nbsp; Register now
```

## cnv_1jc2mqud

**Question**

```text
Hi Marcy,

I work with the certification team at IAAP. I have a certification holder who is
asking if we award education credits for your workshops. After reviewing your
website, it definitely seems like something we’d want to do. We usually try to
get an idea of duration of the workshops in order to know how many credits to
award. I don’t see that type of information on the website. Do you have a
recommendation for the # of hours for each workshop?

Best,

Ellen
```

**Answer**

```text
Hey Ellen


Each workshop's time varies, but typically a single workshop can be completed within a week if you dedicate 2-3 hours per day.

With 6 workshops in the Professional tier, you can expect to finish within 6-8 weeks. This is a rough estimate though and time for completion is subjective depending on the individual's pace.
```

## cnv_nefhykn

**Question**

```text
Hi, I am looking to take the Testing Accessibility training with learning and development funds from my organization. The organization requires the training to provide an Address, Telephone Number, and number of hours for the training. Would you be able to provide that so I can get it approved?

Best,
Joe
```

**Answer**

```text
Hello,


If you're still interested I can provide the info!


Best,
```

## cnv_e6d2scn

**Question**

```text
Hello -
I am with Bright Horizons and we manage the tuition assistance program for Booz Allen Hamilton employees. I have an employee who has submitted for the Epic React Pro for $599. The question I have, I see that the partici[pants do receive completion for the 8 Interactive Self-paced workshops. What I wanted to ask is, do the workshops have any time restrictions as to when they need to be completed? Could it take a participant over a year? And for the "yours forever" part, is that the additional components that are listed including the Bonus Interviews?

We are trying to assist the employee how to properly submitting their application and want to be sure we have all of the information to assist them.

Thank you in advance for your help.

Tanya Kurkowski (she/her/hers)
Senior Supervisor, Employee Services | Bright Horizons EdAssist Solutions
E: [EMAIL]<mailto:[EMAIL]>
[cid:image003.png@01D45FD9.7D360060]  [cid:image007.png@01D3DE40.B07CF9D0] <https://www.linkedin.com/company/edassist>  [cid:image008.png@01D3DE40.B07CF9D0] <https://www.facebook.com/EdAssist>  [cid:image009.png@01D3DE40.B07CF9D0] <https://twitter.com/edassist_bh>  [cid:image010.png@01D3DE40.B07CF9D0] <http://blogs.brighthorizons.com/saw/category/tuition-assistance/>

Out of Office: 3/24-4/1


Internal Reference# [PHONE]

Bright Horizons Children's Centers LLC., 2 Wells Ave. Newton, MA 02459 USA
Bright Horizons Family Solutions Limited (Co. No. [PHONE]), r/o 2 Crown
Court, Rushden, Northamptonshire NN10 6BS UK

This e-mail and any attachments to it are intended only for the addressee
and may contain information which is confidential and/or legally privileged.
If you are not the intended recipient, you are hereby notified that any
dissemination, distribution or copying is strictly prohibited. If you have
received this e-mail in error, please contact the sender immediately and
delete the material from any computer and any printout thereof. Bright
Horizons cannot guarantee that this e-mail communication is secure or free
of error. No liability is accepted for any viruses or for any corruption,
amendment to or deletion of or from this e-mail.
```

**Answer**

```text
Hey Tanya,


The Epic React Pro course is designed to be (once purchased) completed however the user sees fit - So the workshops do not have any time restrictions for when they need to be completed and the user can access the course whenever they would like for however long they need it which makes it yours forever. 


Please let me know if you have any further questions!
```

## cnv_io2yybr

**Question**

```text
Congrats for releasing this course!

My employer has a system for sponsoring such courses. It is required however to
say how long I will need to finish this course. This is a self paced one. But
how many hours on average are needed to finish this course?

After finishing this course, I have to send in the certificate.

Carl Assmann

FYI: I’m in German military currently and they have a program to support
soldiers getting fit for the time after servance. Might be relevant for your
target audience :)
```

**Answer**

```text
Your guess is as good as mine. The factors involved are largely dependent on your skill level and interest. 


Let's say 20 hours.
```

## cnv_hknvgp3

**Question**

```text
Hi,

Will there be any credit or degree provided after the course completion?
because it is required for me to reimburse the course fee from my company.

Thanks
Srikanth
```

**Answer**

```text
Just what is displayed on the site.
```
