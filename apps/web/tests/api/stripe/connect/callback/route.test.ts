...
describe('OAuth callback route', () => {
	...
	it('should handle expired code error', async () => {
		...
		mockOauthToken.mockRejectedValue(new Stripe.errors.StripeInvalidGrantError())

		const req = new NextRequest(
			'http://localhost:4100/api/stripe/connect/callback?code=expired_code&state=same-state',
		)
