...
describe('gather step', () => {
  ...
  describe('extractEmail', () => {
    it('extracts valid email from text', () => {
      const text = 'Please contact me at test@example.com for help'
    ...
    it('returns first email if multiple present', () => {
      ...
      expect(extractEmail(text)).toBe('alice@test.com')
    })
    ...
    it('filters out noreply addresses', () => {
      const text = 'noreply@service.com and customer@real.com'
      expect(extractEmail(text)).toBe('customer@real.com')
    })
  ...
  describe('determineCustomerEmail', () => {
    ...
    it('handles empty sender string', () => {
      const result = determineCustomerEmail('', 'body@example.com')
      expect(result.email).toBe('body@example.com')
      expect(result.source).toBe('body')
    })
  })
        ...
        licenseInfo: [
          {
            ...
            claimedSeats: 7,
            availableSeats: 3,
            claimedBy: [{ email: 'alice@acme.com', claimedAt: '2025-01-15' }],
          },
        ],
      }
