...
describe('createCommentContextService', () => {
  ...
  describe('getCommentThread', () => {
    ...
    it('returns thread with single message', async () => {
      ...
      expect(thread.latestTimestamp).toBe(1700000000)
    })
    ...
    it('returns thread with multiple messages sorted by creation time', async () => {
      ...
      expect(thread.latestTimestamp).toBe(1700000200)
    })
        ...
        _results: [
          createMockMessage({
            ...
            recipients: [
              ...
              { handle: 'support@company.com', role: 'to' },
            ],
          }),
        ],
        _pagination: {},
    ...
    it('uses author email when author is present', async () => {
      ...
      expect(thread.messages[0]?.authorEmail).toBe('agent@company.com')
