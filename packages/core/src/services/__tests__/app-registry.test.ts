...
describe('app-registry', () => {
  ...
  beforeEach(() => {
    mockFindFirst = database.query.AppsTable.findFirst as ReturnType<
      typeof vi.fn
    >
  ...
  describe('getAppByInboxId', () => {
    it('returns null for unknown inbox id', async () => {
      mockFindFirst.mockResolvedValue(undefined)
      const result = await getAppByInboxId('unknown')
      expect(result).toBeNull()
    })

    it('returns app from database', async () => {
      mockFindFirst.mockResolvedValue(mockApp)
      const result = await getAppByInboxId('inbox-123')
      expect(result).toEqual(mockApp)
      expect(mockFindFirst).toHaveBeenCalledTimes(1)
    })
    ...
    it('uses cache from slug-based lookup', async () => {
      ...
      // Second call via inbox id should use cache
      const result = await getAppByInboxId('inbox-123')
      expect(result).toEqual(mockApp)
      expect(mockFindFirst).toHaveBeenCalledTimes(1)
    })
    ...
    it('cache expires after TTL', async () => {
      ...
      await getAppByInboxId('inbox-123')
      expect(mockFindFirst).toHaveBeenCalledTimes(1)
