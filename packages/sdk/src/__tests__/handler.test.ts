...
describe('createSupportHandler', () => {
  ...
  function createRequest(
    ...
    const signature =
      ...
      (options.skipSignature ? '' : createSignature(timestamp, bodyString, secret));
  ...
  describe('action routing', () => {
    it('routes lookupUser action', async () => {
      ...
      expect(mockIntegration.lookupUser).toHaveBeenCalledWith('[EMAIL]');
    });
  ...
  describe('optional methods', () => {
    it('routes getSubscriptions when implemented', async () => {
      ...
      expect(integrationWithSubscriptions.getSubscriptions).toHaveBeenCalledWith(
        'usr_123',
      );
    });
